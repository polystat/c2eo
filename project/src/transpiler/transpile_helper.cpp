...
EOObject GetEODeclRefExpr(const DeclRefExpr *op) {
    ...
    try {
        ...
        if (strcmp(val->getDeclKindName(), "Var") == 0) {
            auto id = dyn_cast<VarDecl>(val);
            const Variable &var = transpiler.glob_.GetVarById(id);
            return EOObject{var.alias};
        } else if (strcmp(val->getDeclKindName(), "EnumConstant") == 0) {
            auto id = dyn_cast<EnumConstantDecl>(val);
            auto x = transpiler.enum_manager_.enum_types;
...
//        auto id = dyn_cast<VarDecl>(val);
//        auto x = val->getDeclKindName();
//        auto y = dyn_cast<EnumConstantDecl>(val);
//        auto n = val->getNameAsString();
//        auto dn = val->getDeclName();
//        const Variable &var = transpiler.glob_.GetVarById(id);
//        return EOObject{var.alias};
