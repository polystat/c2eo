...
if __name__ == '__main__':
    ...
    # Получение каталога, в котором содержатся конфигурационные файлы
    # Проверка числа аргументов командной строки
    argc = len(sys.argv)
    argv = sys.argv
    # При одном аргументе запускается задается одна конфигурация из каталога configuration
    if argc == 2:
    ...
    print('Test Started')
    testedDirNameList = []  # Список имен каталогов
    testedName = configFile.readline()[0:-1]
    # Заполнение списка чтаемых каталогов перед последующей их обработкой
    while testedName:
        # Проверка на существование данного каталога
        testedDir = tmpDir + '/tests/' + testedName
        if os.path.exists(testedDir):
            print(f'Tested Directory is: {testedDir}')
            # Добавление имени каталога в список каталогов
            testedDirNameList.append(testedName)
        else:
    ...
    # Сортировка списка для последующей обработки (в принципе необязательна в текущей реализации)
