...
struct node* incl(n) char* n; { // включение в список нового элемента
    ...
    if((p_tmp = (struct node *)malloc(nodeSize))==NULL)
    {
      er(23); exit(1);
    }
    if((p_tmp->val.name = (char *)malloc(sizeof(char)*(strlen(n)+1)))==NULL)
    {
      er(23); exit(1);
    }
    ...
    if(last == NULL) {// список пуст
      p_tmp->next = p_tmp; // замкнули кольцо
...
void cppOutput(outfil)
    FILE* outfil; 
{
    fprintf(outfil, "#include <iostream>\n");
    fprintf(outfil, "using namespace std;\n\n");
    struct node* p_tmp;
    fprintf(outfil, "//  Описание переменных программы\n");
    if(last == NULL) {
        fprintf(outfil, "// Переменные отсутствуют!\n");
        return;
    }
    p_tmp = last;
    do {
        ...
        fprintf(outfil, "%s%s", typ, p_tmp->next->val.name);
        if(p_tmp->next->val.appl.len > 0) {
            fprintf(outfil, "[%d];\n", p_tmp->next->val.appl.len);
        } else {
            fprintf(outfil, ";\n");
        }
